...
// If the menu is inactive, clear it.
void ui_clear_menu(void) {
    // Copied from ui_draw_menu() cause we want the same area cleared
    int menu_x = 255-100;
    int menu_w = 100;
    int menu_y = 0;
    int menu_h = 192/3*2;
    ...
    // FIXME: save previous contents in ui_draw_menu() and then restore it here. Or something.
}
...
uint32_t ui_handle_key_press(void) {
    ...
    int key_pressed = -1;
    for (int j = 0; ;j += 3) {
        if (km[j] == KEY_END) { // No active GPIOs found, now let's check for PicoCalc keys
#ifdef PICOCALC
            if(cur_key_state.state == PICOCALC_KEY_STATE_PRESSED || 
               cur_key_state.state == PICOCALC_KEY_STATE_HOLD) {
                key_pressed = cur_key_state.code==PICOCALC_KEY_ENTER ? KEMPSTONE_FIRE : cur_key_state.code;
                if(key_pressed == PICOCALC_KEY_ESC) { // Dismiss menu on ESC key
                    cur_key_state.state = PICOCALC_KEY_STATE_IDLE;
                    cur_key_state.code = 0;
                    ui_clear_menu();
                    EMU.menu_active = 0;
                    EMU.menu_left_at_tick = EMU.tick;
                    return UI_EVENT_DISMISS; 
                }
                if(key_pressed < KEMPSTONE_UP) // Only arrow keys, ignore others
                    key_pressed = -1;
            }
#endif
            break;
...
void init_emulator(void) {
    ...
    // Re-initialize UART with desired baud rate at new clock
    ...
    if (SPEAKER_PIN != -1) {
        ...
        // Set PWM wrap value for frequency control
        pwm_config_set_wrap(&config, 9999);  // For example, 10 kHz base frequency assuming 125 MHz clock
...
#ifdef PICOCALC
    // Set joystick type to Kempston for Picocalc arrow keys
    zx_set_joystick_type(&EMU.zx, ZX_JOYSTICKTYPE_KEMPSTON);
#endif
...
#ifdef PICOCALC
        // Read the PicoCalc keyboard and map keys to Spectrum keys
        picocalc_read_kbd(&EMU.zx, EMU.tick);
#endif

        // Handle key presses on the physical device. Either translate
...
#else
        // Pass physical keyboard and GPIOs to Spectrum key registers
        picocalc_handle_zx_key_press(&EMU.zx);
        picocalc_handle_zx_joystick_press(&EMU.zx);
#endif
